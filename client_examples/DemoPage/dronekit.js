var Drone = function (ws_url, latency, APIpassword, messageHandler, failedPasswordCallback) {
    ...
    this.socket.onclose = function (msg) {
        this.callbacks.socket.onclose(msg)
    }
    this.validate = {
        status: false,
    }
...
function connect() {
    vehicle = new Drone($(".ws-url").val(), 500, $(".api-password").val(), function (e) {}, function () {
        alert("Password refused! No API access.")
    })
    
    setInterval(function(){
        $(".heading").html("Heading: " + vehicle.vehicle.attributes.heading + " degrees")
    }, 500)
}
